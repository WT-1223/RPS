...
class LawyerSuspectorEnv(gym.Env):
    ...
    def step(self, lawyer_prompt):
        """根据律师的策略进行对话并更新状态。"""
        self.current_step += 1
        done = self.current_step == self.max_steps

        lawyer_output, suspect_output = self._generate_dialogue(lawyer_prompt)

        self._update_chat_history(lawyer_output, suspect_output, lawyer_prompt)
        dialog_vector = self._encode_dialogue()
        reward = self._calculate_reward()
        self._print_dialogue_info(lawyer_prompt, lawyer_output, suspect_output, reward)
        self._save_dialogue_to_db()
        return dialog_vector, reward, done, {}
